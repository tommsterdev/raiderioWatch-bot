...
async def get_score(args: List[str]) -> str:
    ...
    return f"{res['name'].capitalize()}, {res['realm'].capitalize()} m+ score {res['score']}"
...
async def request_score(query: List[str]) -> Dict[str, Any]:
    ...
    try:
        response = http.request(method='GET', url=endpoint, body=params ,headers={'Content-Type': 'application/json'})
